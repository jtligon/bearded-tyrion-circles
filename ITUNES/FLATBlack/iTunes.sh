		...
		then
			...
			then
				...
				then
					...
					h=`sips --getProperty pixelHeight $img | tail -n1 | awk '{print $2}'`
					x=`sips --getProperty pixelWidth $img | tail -n1 | awk '{print $2}'`
					if [ $h -gt $x ]
					then
					ix="${x}"
					elif [ $h -lt $x ]
					...
					$convert $img -gravity Top -crop ${ix}x${ix}+0+0 +repage $img
					$convert $img -resize 200x $img 
					$convert -size 200x200 xc:none -fill $img  -draw "circle 100,100 100,1" $img 
			fi
			p=`echo $itnesstat`
			z=`echo "3.6 * ${p}" | bc`;y=`echo "-90 + $z" | bc`;
			if [ "$p" = "100" ]
			then
				$convert -size 700x300 xc:transparent -stroke none -fill Black -draw "arc 240,270 0,30 0,360" -stroke White -strokewidth 15 -fill none -draw "arc 230,260 10,40 0,360" $out
				$composite -geometry -230 -gravity center $tcover $out $out
			elif [ "$p" = "0" ]
			then
				$convert -size 1x1 xc:transparent $out
			else
				$convert -size 700x300 xc:transparent -stroke none -fill Black -draw "arc 240,270 0,30 0,360" -stroke White -strokewidth 15 -fill none -draw "arc 230,260 10,40 -90,${y}"  $out
				$composite -geometry -230 -gravity center $tcover $out $out
			fi
