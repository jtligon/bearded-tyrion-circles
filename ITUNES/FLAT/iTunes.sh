...
itnsplst=`osascript -e "tell application \"iTunes\"" -e "return (get player state as string)" -e "end tell"`
		...
		then
			...
			if [ "$p" = "100" ]
			then
				$convert -size 700x300 xc:transparent -stroke none -fill gray -draw "arc 240,270 0,30 0,360" -stroke White -strokewidth 15 -fill none -draw "arc 230,260 10,40 0,360" $out
				$composite -geometry -230 -gravity center $tcover $out $out
			elif [ "$p" = "0" ]
			then
				$convert -size 1x1 xc:transparent $out
			else
				$convert -size 700x300 xc:transparent -stroke none -fill Lightgray -draw "arc 240,270 0,30 0,360" -stroke White -strokewidth 15 -fill none -draw "arc 230,260 10,40 -90,${y}"  $out
				$composite -geometry -230 -gravity center $tcover $out $out
			fi
