...
itnsplst=`osascript -e "tell application \"iTunes\"" -e "return (get player state as string)" -e "end tell"`
		...
		then
			...
			then
				...
				if [ "$itnesstat" = "0" ]
				then
				$convert $out -size 700x300  -stroke White -strokewidth 15 -draw "stroke-linecap square   path 'M 260,240 L 460,240'"  $out
				else
				$convert $out -size 700x300  -stroke White -strokewidth 15 -draw "stroke-linecap square   path 'M 260,240 L 460,240'" -stroke lightgray -strokewidth 15 -draw "stroke-linecap square   path 'M 260,240 L ${perc},240'" $out
				fi 	
			fi
		else
			if [ "$lasttrack" != "" ]
