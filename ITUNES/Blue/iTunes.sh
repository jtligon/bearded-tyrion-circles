...
itnsplst=`osascript -e "tell application \"iTunes\"" -e "return (get player state as string)" -e "end tell"`
		...
		then
			...
			then
				...
				if [ -f ~/Documents/CIRCLE/ITUNES/iTunes-Cover.png ]
				then			    
					...
					$convert $cover -resize 200x $cover 
					$convert -size 200x200 xc:none -fill $cover  -draw "circle 100,100 100,1" $cover  
					mv $cover $tcover
 									else 
 					ln -sf ~/Documents/CIRCLE/ITUNES/$theme/default.png $tcover
				fi
			fi
			p=`echo $itnesstat`
			...
			if [ "$p" = "100" ]
			then
				$convert -size 700x300 xc:transparent -stroke DodgerBlue2 -strokewidth 10 -fill none -draw "arc 230,260 10,40 0,360"  $out
				$composite -geometry -230 -gravity center $tcover $out $out
			elif [ "$p" = "0" ]
			then
				$convert -size 1x1 xc:transparent $out
			else
				$convert -size 700x300 xc:transparent -stroke DodgerBlue2 -strokewidth 10 -fill none -draw "arc 230,260 10,40 -90,${y}"  $out
				$composite -geometry -230 -gravity center $tcover $out $out
			fi
