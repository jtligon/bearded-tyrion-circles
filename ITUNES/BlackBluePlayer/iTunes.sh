...
itnsplst=`osascript -e "tell application \"iTunes\"" -e "return (get player state as string)" -e "end tell"`
		...
		then
			...
			if [ "$track" != "" ]
				then
				echo "\033[1;34m${track}   \033[0m\c"
				printf "\n"
					if [ "$album" != "" ]
					then
						echo "\033[1;34m${album}   \033[0m\c"
						printf "\n"
					else 
					echo "\033[1;34mUNKNOW   \033[0m\c"
					printf "\n"
					fi
					if [ "$artist" != "" ]
					then
						echo "\033[1;34m${artist}   \033[0m\c"
						printf " "
					else 
						echo "\033[1;34mUNKNOW   \033[0m\c"
						printf " "
			...
			then
				...
				then			    
					...
					$convert -size 700x300  xc:transparent -stroke none -fill Gray14 -draw "arc 230,260 10,40 0,360" -stroke none -fill Gray14 -draw "arc 470,260 690,40 0,360" -fill Gray14 -draw "rectangle 120,260 580,40" -stroke darkgray -strokewidth 20 -draw "stroke-linecap square   path 'M 230,230 L 470,230'" -fill darkgray -stroke none  -draw "arc 210,220 230,240 0,360" -fill darkgray -stroke none  -draw "arc 470,220 490,240 0,360" $img
			...
			ae=`echo $wx + 30 | bc`; 
			$convert $img -size 700x300   -fill Blue -stroke none  -draw "arc 210,220 230,240 0,360" -stroke Blue -strokewidth 20 -draw "stroke-linecap square   path 'M 230,230 L ${wx},230'" -fill Blue4 -stroke MidnightBlue -strokewidth 2  -draw "arc ${wx},215 ${ae},245 0,360" $tmp
			mv $tmp $out
		else
			if [ "$lasttrack" != "" ]
			then
