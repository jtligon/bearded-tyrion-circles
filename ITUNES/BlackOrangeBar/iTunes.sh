		...
		then
			...
			then
				...
				then			    
					...
					$convert -size 700x300  xc:transparent -stroke none -fill grey14 -draw "fill-opacity 0.5 arc 421,210 541,90 -90,90" -fill grey14 -draw "fill-opacity 0.5 rectangle 120,210 480,90" $tcover
					#$convert -size 700x300  xc:transparent -stroke none -fill grey14 -draw "fill-opacity 0.5 arc 471,260 691,40 -90,90" -fill grey14 -draw "fill-opacity 0.5 rectangle 120,260 580,40" $tcover 
					$composite -geometry -230 -gravity center $cover $tcover $tcover
					rm -rf $cover
 				else 
					...
					$convert -size 700x300  xc:transparent -stroke none -fill grey14 -draw "fill-opacity 0.5 arc 421,210 541,90 -90,90" -fill grey14 -draw "fill-opacity 0.5 rectangle 120,210 480,90" $tcover
					#$convert -size 700x300  xc:transparent -stroke none -fill grey14 -draw "fill-opacity 0.5 arc 471,260 691,40 -90,90" -fill grey14 -draw "fill-opacity 0.5 rectangle 120,260 580,40" $tcover 
					$composite -geometry -230 -gravity center $tcover $tcover $tcover
				fi
			...
			if [ "$p" = "100" ]
			then
				$convert $tcover -size 700x300 -stroke DarkOrange -strokewidth 10 -fill none -draw "arc 230,260 10,40 0,360"  $out
			elif [ "$p" = "0" ]
			then
				mv $tmp $out
			else
				$convert $tcover -size 700x300 -stroke DarkOrange -strokewidth 10 -fill none -draw "arc 230,260 10,40 -90,${y}" $out
			fi
		else
			if [ "$lasttrack" != "" ]
			then
