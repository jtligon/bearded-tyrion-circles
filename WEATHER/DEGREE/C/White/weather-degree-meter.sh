...
temp=`curl --silent ${url} | grep -E '(Current Conditions:|C<BR)' | sed -e 's/Current Conditions://' -e 's/<br \/>//' -e 's/<b>//' -e 's/<\/b>//' -e 's/<BR \/>//' -e 's/<description>//' -e 's/<\/description>//' -e 's/,//' | tail -n1 | perl -wane 'print $F[-2]'`;
if [ `echo $temp | grep -c "-" ` != 0 ]
then
i=`echo ${#temp}`;let y=$i-1;
m=`echo ${temp:1:$y}`
p=`echo $temp`
z=`echo "6 * ${m}" | bc`;x=`echo "90 - $z" | bc`;y=`echo "180 + $x" | bc`;
...
i=`awk 'BEGIN { print "\xc2\xb0\C"; }'`
	
printf " "
if [ "$p" != "" ]
then
echo "\033[1;37m${p}${i}\033[0m\c"
printf "\n"
fi
